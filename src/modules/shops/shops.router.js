...
router.post('/', auth.authenticate, validate(Joi.object({
  ...
  phone: Joi.string(), email: Joi.string().email(), gstin: Joi.string(),
...
router.post('/:shopId/order-summary', auth.authenticate, function(req, res) {
  ...
  db.one('SELECT balance FROM wallets WHERE user_id=$1', [req.user.id])
    .then(function(w) {
      return shopsService.computeOrderSummary(req.params.shopId, body.items, body.delivery_lat, body.delivery_lng, w ? Number(w.balance) : 0);
    })
